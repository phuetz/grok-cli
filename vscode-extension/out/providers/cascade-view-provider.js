...
exports.CascadeViewProvider = void 0;
const vscode = __importStar(require("vscode"));
class CascadeViewProvider {
    constructor(_extensionUri, aiClient) {
    ...
    async startCascade(task) {
        ...
        try {
            ...
            const maxSteps = config.get('cascadeMaxSteps') || 20;
            const autonomyLevel = config.get('autonomyLevel') || 'confirm';
...
If task is complete, use action "COMPLETE".`,
     ...
     */
    async executeAction(actionData, autonomyLevel) {
        ...
        if (autonomyLevel !== 'full') {
            ...
            if (destructiveActions.includes(action)) {
                const confirm = await vscode.window.showWarningMessage(`Code Buddy wants to: ${actionData.explanation}`, { modal: autonomyLevel === 'confirm' }, 'Allow', 'Skip', 'Stop Cascade');
        ...
        try {
            switch (action) {
                ...
                case 'SEARCH_CODE': {
                    ...
                    await vscode.commands.executeCommand('workbench.action.findInFiles', { query: pattern });
...
exports.CascadeViewProvider = CascadeViewProvider;
CascadeViewProvider.viewType = 'codebuddy.cascadeView';
//# sourceMappingURL=cascade-view-provider.js.map