...
exports.FunctionPredictor = exports.SupercompleteProvider = void 0;
...
class SupercompleteProvider {
    ...
    updateConfig() {
        ...
        this.config.enabled = config.get('inlineCompletions') ?? true;
    }
    async provideInlineCompletionItems(document, position, context, token) {
        ...
        return new Promise((resolve) => {
            ...
            this.debounceTimer = setTimeout(async () => {
                ...
                try {
                    const completion = await this.generateCompletion(document, position, prefix, suffix, token);
     ...
     * Generate cache key
     */
    getCacheKey(document, position, prefix) {
        return `${document.uri.toString()}:${position.line}:${prefix.slice(-100)}`;
    }
    /**
     * Generate completion using AI
     */
    async generateCompletion(document, position, prefix, suffix, token) {
...
Complete the code at <CURSOR>. Output ONLY the completion text, no explanations or markdown.`;
     ...
     */
    detectContext(prefix, _language) {
     ...
     */
    adjustIndentation(completion, prefix) {
        const lines = prefix.split('\n');
        ...
        if (completionIndent.length !== 0 && completionIndent !== currentIndent) {
            ...
            return completionLines
                .map((line, i) => {
                if (i === 0)
                    return line.trim(); // First line continues from cursor
                if (line.trim() === '')
                    return line;
                if (indentDiff > 0) {
     ...
     */
    createCompletionList(completion, position) {
...
exports.SupercompleteProvider = SupercompleteProvider;
/**
...
class FunctionPredictor {
     ...
     */
    async predictFunction(signature, context, language) {
...
Implement the function. Output ONLY the function implementation, no explanations.`;
     ...
     */
    async suggestSignature(functionName, context, language) {
...
exports.FunctionPredictor = FunctionPredictor;
//# sourceMappingURL=supercomplete-provider.js.map